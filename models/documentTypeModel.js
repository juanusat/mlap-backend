...
class DocumentTypeModel {
  static async create({ name, description, code }) {
    const query = `
      INSERT INTO public.document_type (id, name, description, code, active, created_at, updated_at)
      VALUES (
        (SELECT COALESCE(MAX(id), 0) + 1 FROM public.document_type),
        $1, $2, $3, true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
      )
      RETURNING id, name, description, code, active, created_at, updated_at
    `;
  ...
  static async findAll(page = 1, limit = 10) {
    ...
    const query = `
      ...
      LIMIT $1 OFFSET $2
    `;
    const countQuery = `SELECT COUNT(*) FROM public.document_type`;
  ...
  static async search(searchQuery, page = 1, limit = 10) {
    ...
    `;
    const countQuery = `
      SELECT COUNT(*) FROM public.document_type
      WHERE name ILIKE $1 OR description ILIKE $1 OR code ILIKE $1
    `;
  ...
  static async update(id, { name, description, code }) {
    const query = `
      UPDATE public.document_type
      SET name = $1, description = $2, code = $3, updated_at = CURRENT_TIMESTAMP
      WHERE id = $4
      RETURNING id, name, description, code, active, created_at, updated_at
    `;
    const values = [name, description, code, id];
    const result = await db.query(query, values);
    return result.rows[0];
  }
  ...
  static async delete(id) {
    const query = `DELETE FROM public.document_type WHERE id = $1 RETURNING id`;
    const result = await db.query(query, [id]);
    return result.rows[0];
  }
}
